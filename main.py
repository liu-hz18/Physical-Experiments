...
d = 0.734
T = 293.0  # K
gamma = 3.0 * math.pi * d * eta  # 3pi * 0.0001 Pa.s * 10^-6 m = 3pi*10^-10
R = 8.314  # J.mol.K^-1
tau = 30
...
def run(times):
    for t in range(times):
        if 0 != os.system('brown %d > results/out_%d.txt' %
            (random.randint(0, 65535), t)):
...
def static_1(n):
    ...
    corr = np.corrcoef(mean_array, array)[0][1]
    D = A / 2
    k_b = gamma * D / T
    N_a = R / k_b / 100.0
    ...
    plt.legend(['y = {:.4f}t\n$R^2$ = {:.4f}'.format(A, corr)], loc='lower right')
